...
**e. The Phase 2 pack repeats the same mistake at the language level.** The original `session_pack.json` defined `verify` with execution type `set_value`. That is bookkeeping, not verification — the same theater as `claim-basis`. The pack as specified would not improve the skill, it would give the model two ways to lie instead of one. The pack has since been narrowed: nouns ship now, `verify` is removed pending V4-Q1 (see (h) below).

**h. The pack verb contract (v4a §137) cannot express `verify <claim> from <source>`.** This is the structural blocker behind (e) and behind the `cite` recommendation in §1 of Suggested Next Actions. v4a §137 requires every slot to be introduced by a connective — there is no way to declare a direct-object slot that sits immediately after the verb, before any connective. So a verb with the shape `<verb> <direct-object> <connective> <argument>` cannot be registered as a pack verb at all. `verify <claim> from <source>` is exactly this shape, and so is `cite <text> from <source>`. **This is the same gap that blocked the Möbius DSL convergence — see the Möbius rename checkpoint §6, gap #2.** Resolving V4-Q1 (connective-less direct-object slots) unblocks both efforts at once. Until then, `verify` and `cite` cannot ship as pack verbs even if the substring-check execution semantics were added. This is the next-most-important fix to the language, full stop.
